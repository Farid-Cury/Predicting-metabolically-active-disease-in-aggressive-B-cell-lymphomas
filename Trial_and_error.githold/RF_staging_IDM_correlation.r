...
data_rf <- data_nonnormal %>%
  select(estadiamento_cat, Correlation, IDM, Esc_CAT)
...
```{r}
# Calcular a curva ROC para rf_res_2 e adicionar um identificador de modelo
roc_rf_res_2 <- rf_res_2 %>%
  collect_predictions() %>%
  roc_curve(Esc_CAT, .pred_1, event_level = "second") %>%
  mutate(Model = "Training")  # Nomeie o primeiro modelo

# Calcular a curva ROC para final_res_rf e adicionar um identificador de modelo
roc_final_res_rf <- final_res_rf %>%
  collect_predictions() %>%
  roc_curve(Esc_CAT, .pred_1, event_level = "second") %>%
  mutate(Model = "Testing")  # Nomeie o segundo modelo
