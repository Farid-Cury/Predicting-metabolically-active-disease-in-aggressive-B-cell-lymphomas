...
data_prot <- data_nonnormal %>%
  select(estadiamento_cat, HK2tu_opt_cp, Esc_CAT)
...
xgb_model <- boost_tree(
           trees = 500,
           learn_rate = 0.1443436,
           min_n = 5
       ) %>%
           set_engine("xgboost", scale_pos_weight = 1.158156) %>%
           set_mode("classification")

set.seed(12)
xgb_res <- fit_resamples(xgb_model,
                                   rec_spec,
...
final_res_xgb <- last_fit(final_wf_xgb, data_split) 
...
set.seed(12)
final_res_xgb_boot <- int_pctl(final_res_xgb, alpha = 0.05)
final_res_xgb_boot
...
threshold_data_te <- final_res_xgb %>% 
  collect_predictions() %>%
  threshold_perf(Esc_CAT, .pred_1, event_level = "second", thresholds)
