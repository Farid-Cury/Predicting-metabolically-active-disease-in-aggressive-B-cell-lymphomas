---
title: "MTV and IDM - RF Radiomic model"
author: "Farid"
date: "`r Sys.Date()`"
...
data_rf <- data_nonnormal %>%
  select(Semantic_MTV, Texture_IDM, Outcome_5PS_CAT)
...
threshold_data_te %>%
  arrange(.threshold) %>%
  filter(near(.threshold, 0.310) | near(.threshold, 0.395))
```

## Confusion matrix
```{r}
pred_final_res_rf <- final_res_rf %>% 
  collect_predictions() %>%
  mutate(pred_class_youden = ifelse(.pred_1 >= 0.395, 1, 0))
pred_final_res_rf$pred_class_youden <- as.factor(pred_final_res_rf$pred_class_youden)

pred_final_res_rf$Outcome_5PS_CAT <- fct_recode(pred_final_res_rf$Outcome_5PS_CAT, "1-3" = "0", "4-5" = "1")
pred_final_res_rf$pred_class_youden <- fct_recode(pred_final_res_rf$pred_class_youden, "1-3" = "0", "4-5" = "1")
...
```{r}
roc_data_rf_rad_tr <-rf_res_2 %>% 
  collect_predictions()

roc_data_rf_rad_te <- final_res_rf %>% 
  collect_predictions()

library(writexl)
write_xlsx(roc_data_rf_rad_tr, "roc_data_rf_rad_tr.xlsx", col_names = TRUE)
write_xlsx(roc_data_rf_rad_te, "roc_data_rf_rad_te.xlsx", col_names = TRUE)
```
