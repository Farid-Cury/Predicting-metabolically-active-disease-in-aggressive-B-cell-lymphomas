---
title: "Staging - LR Clinical  model"
author: "Farid"
date: "`r Sys.Date()`"
...
## Building the model for 5-PS prediction using Ann Arbor Staging (Clinical model).
```{r}
# selecting the Ann Arbor Staging.
data_rf <- data_nonnormal %>%
  select(CL_Staging, Outcome_5PS_CAT)
...
svm_spec <- svm_rbf(cost = tune(),
                    rbf_sigma = tune()) %>% 
   set_engine("kernlab", class_weights = "balanced") %>% 
   set_mode("classification")
...
pred_lr_res$pred_class_youden <- as.factor(pred_lr_res$pred_class_youden)

pred_lr_res$Outcome_5PS_CAT <- fct_recode(pred_lr_res$Outcome_5PS_CAT, "1-3" = "0", "4-5" = "1")
pred_lr_res$pred_class_youden <- fct_recode(pred_lr_res$pred_class_youden, "1-3" = "0", "4-5" = "1")
...
roc_data_lr_cl_te <- final_res_lr %>% 
  collect_predictions() %>%
  roc_curve(Outcome_5PS_CAT, .pred_1, event_level = "second") %>%
  autoplot() +
  geom_abline(lty = 1, color = "black", size = 0.5) +
...
## Confusion matrix
```{r}
pred_test_res_lr <- final_res_lr %>% 
  collect_predictions() %>%
  mutate(pred_class_youden = ifelse(.pred_1 >= 0.200, 1, 0))
pred_test_res_lr$pred_class_youden <- as.factor(pred_test_res_lr$pred_class_youden)

pred_test_res_lr$Outcome_5PS_CAT <- fct_recode(pred_test_res_lr$Outcome_5PS_CAT, "1-3" = "0", "4-5" = "1")
pred_test_res_lr$pred_class_youden <- fct_recode(pred_test_res_lr$pred_class_youden, "1-3" = "0", "4-5" = "1")
